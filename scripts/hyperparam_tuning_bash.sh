...
ENV="environment_cpu.yml"    # YAML file to build docker image from
...
docker run --rm \
  ...
  "$IMAGE" \
  bash -lc 'pip install -e . && python scripts/setup.py && python scripts/hyperparam_tuning.py' | tee -a "$LOG"
...
# not the src/jfk_taxis dir, then we run the setup.py script (we have already made most of the directories but we need to select the 
# appropriate device for xgboost) then we just run the hyperparam tuning script
