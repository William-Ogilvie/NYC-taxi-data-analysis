...
docker run --rm \
  ...
  "$IMAGE" \
  bash -lc 'pip install -e . && cd scripts && python setup.py && python scripts/hyperparam_tuning.py' | tee -a "$LOG"
...
# not the src/jfk_taxis dir, then we cd into the scripts dir and run the setup.py script (we have already made most of the directories but we need to select the 
